...
struct VariantTopoEdge
{
    ...
    public bool isCurvedBridge;     // gap is bridged by a hidden curved rail; use curvedRatio instead of segRatio
}
...
List<VariantTopoEdge> CaptureBalustradeTopology(GameObject root, string currentVariant)
{
    ...
    var snapsOf = new Dictionary<int, List<(Transform snap, string name)>>();
    ...
    {
        int idA = objects[i].GetInstanceID();
        for (int j = i + 1; j < objects.Count; j++)
        {
            int idB = objects[j].GetInstanceID();
            foreach (var (sa, na) in snapsOf[idA])
            {
                foreach (var (sb, nb) in snapsOf[idB])
                {
                    if (Vector3.Distance(sa.position, sb.position) < tol)
                    {
                        ...
                        long pairKey = ((long)lo << 32) | ((long)hi & 0xFFFFFFFFL);
    ...
    foreach (Transform child in root.transform)
    {
        ...
        foreach (var go in objects)
        {
            ...
            foreach (var (s, n) in snapsOf[go.GetInstanceID()])
            {
        ...
        int idGA = pA.GetInstanceID();
        int idGB = pB.GetInstanceID();
        int gLo = idGA < idGB ? idGA : idGB, gHi = idGA < idGB ? idGB : idGA;
        long gKey = ((long)gLo << 32) | ((long)gHi & 0xFFFFFFFFL);
        if (paired.Add(gKey))
        {
            edges.Add(new VariantTopoEdge
            {
    ...
    foreach (var go in objects)
        parent[go.GetInstanceID()] = go.GetInstanceID();
    foreach (var e in edges)
        Union(e.idA, e.idB);

    // Group pillars by component (using centers, NOT snaps)
    var compPillars = new Dictionary<int, List<GameObject>>();
    foreach (var go in objects)
    {
        if (!IsPillarInstance(go)) continue;
        int comp = Find(go.GetInstanceID());
    ...
    var compKeys = new List<int>(compPillars.Keys);
    ...
    var gapUF = new Dictionary<int, int>();
    foreach (var k in compPillars.Keys) gapUF[k] = k;
    int GFind(int x) { while (gapUF[x] != x) { gapUF[x] = gapUF[gapUF[x]]; x = gapUF[x]; } return x; }

    foreach (var (dist, goA, goB) in gapCandidates)
    {
        int cA = GFind(Find(goA.GetInstanceID()));
        int cB = GFind(Find(goB.GetInstanceID()));
        if (cA == cB) continue;
        gapUF[cA] = cB;
        ...
        edges.Add(new VariantTopoEdge
        {
            idA = goA.GetInstanceID(), idB = goB.GetInstanceID(),
...
void RepositionByTopology(GameObject root, List<VariantTopoEdge> edges,
    Dictionary<int, GameObject> swapMap, string fromVariant, string toVariant)
{
    // Resolve old IDs to new GameObjects
    GameObject Resolve(int oldId)
    {
        if (swapMap.TryGetValue(oldId, out var g) && g) return g;
        return EditorUtility.InstanceIDToObject(oldId) as GameObject;
    }
...
void ReplaceBalustradeVariant(string fromTag, string toTag)
{
    ...
    var swapMap = new Dictionary<int, GameObject>();
    var replaceList = new List<(GameObject go, GameObject prefab)>();
